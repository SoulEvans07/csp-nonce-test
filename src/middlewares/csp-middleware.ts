...
const trimCspRules = (rules: string): string => rules.replace(/\s{2,}/g, ' ').trim();
export const getCspRules = (nonce: string, isDev: boolean): string => {
  const strict = isDev ? '' : `'nonce-${nonce}' 'strict-dynamic'`;

  const tescoHosts = ['*.tesco.com', '*.tesco.ie', '*.itesco.cz', '*.tesco.hu', '*.tesco.sk'];

  const httpsTescoHosts = tescoHosts.map((host) => `htpps://${host}`);

  return trimCspRules(`
    default-src 'self' ${tescoHosts.join(' ')} dummyjson.com graphqlzero.almansi.me;
    script-src 'self' 'unsafe-inline' 'unsafe-eval' ${httpsTescoHosts.join(' ')} ${strict};
